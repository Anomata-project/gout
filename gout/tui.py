...
class Tui:
    ...
    def sheet_build(self) -> list[dict]:
        ...
        for t in p.tracks():
            ...
            row(f"t{n}:in", "in", fmt_ms(a), lambda v, n=n: ["trim", n, "-st", v], "soft trim in, file time; -5s from its end")
            row(f"t{n}:out", "out", fmt_ms(b), lambda v, n=n: ["trim", n, "-et", v], "soft trim out, file time; -5s from its end")
